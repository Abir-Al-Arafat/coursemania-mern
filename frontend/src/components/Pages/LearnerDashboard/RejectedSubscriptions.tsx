...
const RejectedSubscriptions: React.FC = () => {
  ...
  const handleSubscribeAgain = (course: Course) => {
    setSelectedCourse(course);
    setShowModal(true);
  };

  const handleConfirmSubscribeAgain = (confirmed: boolean) => {
        ...
        >
          ...
          <div className="p-4">
            ...
            <div className="flex justify-between items-center mt-4">
              <button
                className={`text-blue-500 hover:text-blue-600 mr-4 flex items-center focus:outline-none`}
                onClick={() => handleSubscribeAgain(course)}
              >
                Subscribe Again
              </button>
              <button
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-md shadow-md">
            <p>
              Are you sure you want to subscribe to {selectedCourse?.title}{" "}
              again?
            </p>
            <div className="mt-4 flex justify-end">
              <button
                className="bg-blue-500 text-white px-4 py-2 rounded-md mr-4"
                onClick={() => handleConfirmSubscribeAgain(true)}
              >
                Yes
              </button>
              <button
                className="bg-gray-300 text-gray-800 px-4 py-2 rounded-md"
                onClick={() => handleConfirmSubscribeAgain(false)}
              >
                No
              </button>
            </div>
